...
- **Find** — a single top-level **FullVid** context-menu item on any game opens the video dialog directly and searches YouTube via yt-dlp. Search is async and non-blocking: the results window opens instantly showing "Searching…" and results stream in as they resolve — no blocking progress dialog. Each result carries thumbnail, title, duration, channel, and view count.
